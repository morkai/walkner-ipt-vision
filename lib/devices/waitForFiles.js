...
module.exports = (app, stepConfig, stepResult, done, startedAt) =>
{
  ...
  if (!startedAt)
  {
    app.log('Waiting for FTP uploads...');

    startedAt = now;
  }

  const uploadedFiles = fs.readdirSync(app.config.ftp.root)
    .filter(file => /\.bmp$/i.test(file))
    .map(file =>
    {
  ...
  const waitForFilesMs = (app.config.ftp.waitForFiles || 10000) + 100;

  if (!uploadedFiles.length && now - startedAt >= waitForFilesMs)
  {
    app.log('...waiting for FTP uploads expired.');
  ...
  const overlayPath = imagePath.replace(/\.bmp$/i, '.svg');
