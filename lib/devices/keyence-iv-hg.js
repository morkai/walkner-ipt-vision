...
module.exports = (app, {stepConfig, device}, done) =>
{
  ...
  const command = path.join(app.root, 'bin', 'KeyenceTest', 'KeyenceTest.exe');
  const args = [
    '--host', device.host,
    '--port', device.port || 44818,
    '--program', stepConfig.program || 0,
    '--tools', stepConfig.tools || 0
  ];
  const options = {};
  ...
  p.on('error', err =>
  {
    complete({
      code: 'ERR_SPAWN',
      message: `KeyenceTest.exe spawning error: ${err.message}`
    });
  });
  ...
  p.on('close', code =>
  {
    if (code === 0)
    {
      try
      {
        Object.assign(stepResult, JSON.parse(stdout.replace(/EEIP.*?Solutions/s, '')));

        if (stepResult.tools)
        {
          stepResult.tools = stepResult.tools.filter(
            tool => tool.result
              || tool.matchingRate !== 0
              || tool.lowerThreshold !== 0
              || tool.upperThreshold !== 65535
          );
        }
    ...
    else
    {
      complete({
        code: errorCode,
        message: `KeyenceTest.exe ended with a non-zero exit code: ${code}`
      });
    }
  });
};
